...
Read-side counterpart: src/lib/storage-paths.ts. Every path surfaced on the TS
side must also be buildable here so write-side scripts don't drift. A few
paths (matrix, og_descriptions) are write-only for now and have no TS
counterpart — intentional, not an oversight.
"""
