 ...
 * site ±2 bp) → filled amber `Badge`; weak-only tier (UTR / intron /
 * ±2 kb flank) → outline. Neither → nothing rendered.
 *
...
export function SvOverlapBadge({ entry }: Props) {
  ...
  if (entry.s === 1) {
    ...
    return (
      <Badge
        variant="warning"
        title={tip}
        className="text-[9px] font-mono uppercase tracking-wide px-1 py-[1px] rounded border-amber-400 bg-amber-100 text-amber-900 h-auto"
      >
        SV
      </Badge>
    );
  ...
  return (
    <Badge
      variant="outline"
      title={weakTip}
      className="text-[9px] font-mono uppercase tracking-wide px-1 py-[1px] rounded border-gray-300 bg-white text-gray-500 h-auto"
    >
      SV·w
    </Badge>
  );
