...
| TD-009 | types/services | IRGSP reference cluster is a pseudo-cluster that reuses OG-level `afSummary` instead of a per-cluster v2 graph/AF bundle. Keep branching explicit via `GeneCluster.source`. Promote to real pipeline extraction with IRGSP anchor if IRGSP becomes a primary workflow. | Medium | 2026-04-18 |
...
| TD-R005 | scripts/types | Retired versionless `og_region/...` path helpers and v1 live uploader | 2026-04-26 | v2 pointer/graph/AF paths are covered by cross-language parity |
