...
export async function parseFasta(filePath: string): Promise<AssemblyStats> {
  ...
  const chrNames = seqNames.filter(
    (n) => /^chr/i.test(n) || /^[0-9]+$/.test(n),
  );
...
function computeN50(sortedDesc: number[], total: number): number {
  const half = total / 2;
  let cumulative = 0;
